...
class NewsAdapter(newsList: List<Article?>): RecyclerView.Adapter<NewsAdapter.NewsHolder>() {
    ...
    override fun onBindViewHolder(holder: NewsHolder, position: Int) {
        ...
        holder.itemView.apply {
            Glide.with(this).load(article?.image).into(holder.ivArticleImage)
            holder.tvTitle.text = article?.title
            holder.tvDescription.text = article?.description
            holder.tvSource.text = article?.source?.name
            TextPattern.checkDateForPattern(holder.tvPublishedDate)
            holder.tvPublishedDate.text = article?.publishedDate
        }
    }
